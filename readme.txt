=== Hulvire Fotofolio ===
Contributors: (this should be a list of wordpress.org userid's)
Donate link: http://amfajnor.sk/
Tags: foto, portfolio, photo, gallery, photo folio,
Requires at least: 3.0.1
Tested up to: 4.1
Stable tag: 1.2.1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Plugin to show simple fotofolio.

== Description ==

Plugin for displaying fotofolio with simple picture gallery on wordpress sites. 
Simple text entry and thumbnail pictures, option to add multiple images.


== Installation ==

1. Upload `hulvire-fotofolio` folder to the `/wp-content/plugins/` directory
1. Activate the plugin through the 'Plugins' menu in WordPress
1. Place in your pages
`[huu_fotofolio]` or `[huu_fotofolio id=“your gallery id”]`
1. in themes
`huu_fotofolio();` or `huu_fotofolio(id=“your gallery id”)`


This plugin needs for proper function Meta Box plugin installed.


== Screenshots ==

1. This is settings of plugin
2. This is plugin in wordpress page

== Changelog ==

= 1.5 =
* Additional settings for slider, swipe functionality for mobile devices


= 1.4 =
* Many major fixes, added more galleries possibility true shortcode id definition


= 1.3 =
* Removed major bug which not allow the activation of more than one hulvire plugin


= 1.2 =
*
*


= 1.0 =
* A change since the previous version.
* first construct

